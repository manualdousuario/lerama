        ...
        <div class="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-x-4 px-4">
            <div class="flex flex-wrap items-center gap-x-5">
                <div class="flex items-center py-2">
                    @auth
                        <i class="ti ti-lock text-xl text-topbar-text" aria-hidden="true"></i>
                    @else
                ...
                <nav class="flex flex-wrap items-center gap-x-1">
                    @auth
                        <x-nav-link href="/admin" :active="$active === 'admin'" icon="ti-news">{{ __('nav.articles') }}</x-nav-link>
                        <x-nav-link href="/admin/feeds" :active="$active === 'admin-feeds'" icon="ti-layout-grid">{{ __('nav.feeds') }}</x-nav-link>
                        <x-nav-link href="/admin/categories" :active="$active === 'admin-categories'" icon="ti-folder">{{ __('nav.categories') }}</x-nav-link>
                        <x-nav-link href="/admin/tags" :active="$active === 'admin-tags'" icon="ti-tags">{{ __('nav.topics') }}</x-nav-link>
                        <x-nav-link href="/admin/logout" icon="ti-logout">{{ __('nav.logout') }}</x-nav-link>
                    @else
