...
<body class="flex min-h-screen flex-col bg-paper text-ink dark:bg-night dark:text-night-ink">
        ...
        <div class="mx-auto max-w-6xl px-4">
            <div class="flex items-center justify-between gap-x-4">
                <div class="flex items-center gap-x-5">
                    ...
                    <nav class="hidden items-center gap-x-1 md:flex">
                ...
                <div class="flex items-center gap-2 py-2">
                    ...
                    <button id="darkModeToggle" class="btn-sm btn-topbar py-2" type="button" aria-label="{{ __('a11y.toggle_dark_mode') }}" aria-pressed="false">
                        <i class="ti ti-sun hidden" id="lightIcon" aria-hidden="true"></i>
                        <i class="ti ti-moon" id="darkIcon" aria-hidden="true"></i>
                    </button>
                    <button id="mobileMenuToggle" class="btn-sm btn-topbar py-2 md:hidden" type="button" aria-label="{{ __('a11y.toggle_menu') }}" aria-expanded="false" aria-controls="mobileMenu">
                        <i class="ti ti-menu-2" id="menuOpenIcon" aria-hidden="true"></i>
                        <i class="ti ti-x hidden" id="menuCloseIcon" aria-hidden="true"></i>
                    </button>
                </div>
            </div>
            <nav id="mobileMenu" class="hidden flex-col gap-1 pb-3 md:hidden" aria-label="{{ __('nav.home') }}">
                <x-nav-link href="/" :active="$active === 'home'" icon="ti-home" class="w-full">{{ __('nav.home') }}</x-nav-link>
                <x-nav-link href="/feeds" :active="$active === 'feeds'" icon="ti-rss" class="w-full">{{ __('nav.feeds') }}</x-nav-link>
                <x-nav-link href="/suggest-feed" :active="$active === 'suggest-feed'" icon="ti-circle-plus" class="w-full">{{ __('nav.suggest') }}</x-nav-link>
                <x-nav-link href="/shuffle" :active="$active === 'shuffle'" icon="ti-refresh" class="w-full">{{ __('nav.shuffle') }}</x-nav-link>
                <x-nav-link href="/random" target="_blank" icon="ti-arrow-up-right" class="w-full">{{ __('nav.random') }}</x-nav-link>
            </nav>
        </div>
    </nav>
    ...
    <footer class="mt-auto border-t border-line py-6 dark:border-night-line">
        <div class="mx-auto max-w-6xl px-4">
            ...
            <div class="mt-3 grid grid-cols-2 gap-2 sm:flex sm:flex-wrap sm:justify-center">
                <a href="https://github.com/manualdousuario/lerama" target="_blank" class="btn-sm btn-outline" title="GitHub">
                    <i class="ti ti-brand-github" aria-hidden="true"></i> GitHub
                </a>
                <a href="/categories" class="btn-sm btn-outline" title="{{ __('nav.categories') }}">
                    <i class="ti ti-folder" aria-hidden="true"></i> {{ __('nav.categories') }}
                </a>
                <a href="/tags" class="btn-sm btn-outline" title="{{ __('nav.topics') }}">
                    <i class="ti ti-tags" aria-hidden="true"></i> {{ __('nav.topics') }}
                </a>
                <button id="copySeloLerama" class="btn-sm btn-outline" title="{{ __('footer.badge') }}">
